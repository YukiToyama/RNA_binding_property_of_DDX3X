...
ds_pop = np.zeros(len(temp))
ds_pop_e = np.zeros(len(temp))
ss_pop = np.zeros(len(temp))
ss_pop_e = np.zeros(len(temp))

for i in range(len(temp)):
    ds=np.array(data[(data['Temp']==temp[i])].Vol_ds)
    ss=np.array(data[(data['Temp']==temp[i])].Vol_ss)
    ds_pop[i] = np.average(ds/(ds+ss))
    ss_pop[i] = np.average(ss/(ds+ss))
    ds_pop_e[i] = np.std(ds/(ds+ss))
    ss_pop_e[i] = np.std(ss/(ds+ss))
